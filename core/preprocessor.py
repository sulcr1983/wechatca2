...
_SENTENCE_WORDS = re.compile(
    r'(?:是|的|了|在|就|都|很|不|也|我|你|他|她|它|们|这|那|会|能|要|可以|已经|'
    r'因为|所以|但是|如果|虽然|而且|然后|之后|之|与|及|或|把|被|让|给|对|从|到|'
    r'中|上|下|里|外|前|后|左|右|大|小|多|少|高|低|长|短|新|旧|好|坏|快|慢|'
...
def preprocess(text: str) -> str:
    ...
    for line in lines:
        ...
        # 已含 Markdown 标记的行原样保留
        if re.match(r'^(#{1,6}\s|>\s|-\s|\*\s|\d+\.\s|```|~~)', stripped):
            if in_code_block:
                md_lines.append('```')
                in_code_block = False
