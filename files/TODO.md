...
|---|------|--------------------|------|
| U-1 | 预览卡 `<br>` 修复（`join('\n')` + CSS 换行） | 预览卡不再出现字面 `<br>` | ⬜ |
| U-2 | 封面标题溢出：引入 **fitty**（40–76px 自适应），两套引擎模板通用 | 长/短标题 × 3 风格均无溢出、无重叠 | ⬜ |
| U-3 | 生成三阶段进度（找底图→排版→完成）+ 按钮置灰防重复点，**不写假秒数** | 点击后 100ms 内出现进度 | ⬜ |
| U-4 | 底图署名按实际来源显示（local 时写明降级原因） | 断网跑一次，提示正确 | ⬜ |
| U-5 | 输入框示例改「看示例 / 清空」+ **记住上次主题** | 重新打开即恢复上次主题 | ⬜ |
| U-6 | 推送未配置 → 引导至设置；复制 / 推送**同级醒目** | 未配置点推送有明确指引，不卡空下拉 | ⬜ |
